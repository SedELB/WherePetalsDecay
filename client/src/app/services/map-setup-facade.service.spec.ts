...
describe('MapSetupFacadeService', () => {
    ...
    beforeEach(() => {
        ...
        communication = jasmine.createSpyObj<CommunicationService>('CommunicationService', ['createGame', 'modifyGame']);
    ...
    it('creates a new game (thumbnail + validation + API call)', async () => {
        ...
        spyOn(service as any, 'captureThumbnail').and.resolveTo('new-thumb');
    ...
    it('updates an existing game (no thumbnail capture)', async () => {
        ...
        const captureSpy = spyOn(service as any, 'captureThumbnail').and.resolveTo('ignored');
        mapSetup.buildValidationPayload.and.returnValue(validationPayload);
        validator.validate.and.returnValue({ isValid: true, errors: [] });
    ...
    it('shows a clear error when thumbnail generation fails', async () => {
        ...
        spyOn(service as any, 'captureThumbnail').and.rejectWith(new Error('fail'));
        const alertSpy = spyOn(window, 'alert');
    ...
    it('captures a thumbnail when the DOM element exists', async () => {
        ...
        const dataUrl = await (service as any).captureThumbnail('test');
    ...
    it('throws if the thumbnail element is missing', async () => {
        ...
        await expectAsync((service as any).captureThumbnail('test')).toBeRejectedWithError('Thumbnail element not found');
    });
});