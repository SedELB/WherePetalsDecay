...
import { IsometricAnimationService } from '@app/services/isometric-view/isometric-animation.service';
...
export class IsometricMapComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  constructor(
    private isometricViewService: IsometricViewService,
    private animationService: IsometricAnimationService,
  ) {}

  ngOnChanges(): void {
    this.animationService.update(this.playerPositions);
 ...
 this.render(); 
  ...
  private onMouseDown(e: MouseEvent): void {
    if (this.lockCamera || e.button !== 0) return;
  ...
  private onMouseMove(e: MouseEvent): void {
    ...
    if (this.isDragging) {
      this.cameraX = this.cameraStartX + (e.clientX - this.dragStartX) / this.zoom;
      this.cameraY = this.cameraStartY + (e.clientY - this.dragStartY) / this.zoom;
      this.render();
      return;
    }
    const pos = this.getOriginalGridPosition(e);
    const isAction = pos && this.actionHighlightTiles.some(h => h.pos.x === pos.x && h.pos.y === pos.y);
    this.canvasRef.nativeElement.style.cursor = isAction ? 'pointer' : 'default';
  }

  private onMouseUp(): void {
 this.isDragging = false; this.canvasRef.nativeElement.style.cursor = 'default'; 
}
